...
> 這篇文檔是為了Linux用戶而準備的，請某些人高抬貴手別和我槓"怎麼Windows只提了一嘴"
...
現在請您先輸入python --version查看現存Python版本，3.6以下都不可以運行
...
- 如果您已經配置好了Python3.7(或更高)環境，請在終端執行pip -r pipreq以安裝依賴
	...
	- 注2：如果您正在使用Linux請將pip替換為sudo pip3

### Java
> 雖然開服器本身並不需要Java，可Java是開服的必需品，所以在此一併列出其下載方式（僅針對Linux）
#### 使用apt安裝

##### 如您的發行版可以使用apt，請按照您的情況選擇命令（必須要有root權限，推薦直接複製）：
...
##### 如果您的發行版使用不了apt或者上面的命令報錯，請嘗試在這裡下載：
...
- [命令行安裝deb包教程](https://blog.csdn.net/oMcLin/article/details/108725325)
#### Arch特編
##### OpenJDK安裝
...
- 可使用```yay -s (JDK名字後面的括號里的內容，不要帶上括號)```來安裝對應版本
...
您得先有一個域名，然後參照以下教程：
- [阿里雲](https://doc.natfrp.com/#/app/mc?id=srv-aliyun)
...
- ```force-gamemode``` 強制玩家加入時為默認遊戲模式[True/False]
- ```allow-nether``` 是否允許下界[True/False]
- ```enforce-whitelist``` 在服務器上強制使用白名單[True/False]
